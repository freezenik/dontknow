...
/* Vectorized: .Call("pbvnorm_miwa", h, k, rho, steps)
   Returns P(Z1 <= h, Z2 <= k) for standardized BVN with corr=rho.
   Arguments can be scalars or vectors, recycled to common length. */
SEXP pbvnorm_miwa(SEXP H, SEXP K, SEXP RHO, SEXP STEPS)
{
    H     = PROTECT(coerceVector(H,    REALSXP));
    K     = PROTECT(coerceVector(K,    REALSXP));
    RHO   = PROTECT(coerceVector(RHO,  REALSXP));
    STEPS = PROTECT(coerceVector(STEPS, INTSXP));

    R_xlen_t nH = XLENGTH(H), nK = XLENGTH(K), nR = XLENGTH(RHO);
    R_xlen_t n = nH; if(n < nK) n = nK; if(n < nR) n = nR;
    if(n == 0) { UNPROTECT(4); return allocVector(REALSXP, 0); }

    int steps = LENGTH(STEPS) ? INTEGER(STEPS)[0] : 128;
    ...
    SEXP Lower = PROTECT(allocVector(REALSXP, dim));
    double *lower = REAL(Lower); lower[0] = R_NegInf; lower[1] = R_NegInf;

    SEXP Infin = PROTECT(allocVector(INTSXP, dim));
    int *infin = INTEGER(Infin); infin[0] = 0; infin[1] = 0; /* (-Inf, upper] */

    for(R_xlen_t i = 0; i < n; ++i) {
        double hi  = REAL(H)[  nH==1 ? 0 : i ];
        double ki  = REAL(K)[  nK==1 ? 0 : i ];
        double rho = REAL(RHO)[nR==1 ? 0 : i ];

        /* 2x2 correlation (row-major fill, but symmetric so OK) */
        SEXP Corr = PROTECT(allocVector(REALSXP, dim*dim));
        double *corr = REAL(Corr);
        corr[0] = 1.0;  corr[1] = rho;
        corr[2] = rho;  corr[3] = 1.0;

        SEXP Upper = PROTECT(allocVector(REALSXP, dim));
        double *upper = REAL(Upper);
        upper[0] = hi; upper[1] = ki;

        SEXP res = PROTECT(R_miwa(Steps, Corr, Upper, Lower, Infin));
        ans[i] = REAL(res)[0];

        UNPROTECT(3); /* Corr, Upper, res */
    }

    UNPROTECT(8); /* H, K, RHO, STEPS, out, Steps, Lower, Infin */
    return out;
}
